...
void setup()
{
  ...
  // ultrasonic sensor setup, defines pins numbers
  pinMode(echoPin, INPUT);
  pinMode(trigPin, OUTPUT);

  //basic setup of gyroscope
  Wire.begin();
  mpu.begin();
...
long ultrasonic_distance_in_cm;
void ultrasonic_measurement(){
  ...
  ultrasonic_distance_in_cm = (duration/2.0) / 29.1;
}
void loop()
{
    while (true){
      ADVANCE();
      // required function 1 code:
      ultrasonic_measurement();
      Serial.print("ultrasonic_distance_in_cm: ");
      Serial.print(ultrasonic_distance_in_cm);
      Serial.println("");
      delay(10);
      if (ultrasonic_distance_in_cm < 20){
        STOP();
        delay(2000);
  ...
  while(true){
  mpu.update();                                              
    if (mpu.getAngleX() > 85 && mpu.getAngleX() < 95){
      STOP();
  ...
  double current_x_value1 = mpu.getAngleX();
  while(true){
    mpu.update();
    rotate_1();
    if (mpu.getAngleX() > 85 && mpu.getAngleX() < 95 && mpu.getAngleX() > current_x_value1){
    STOP();
    delay(2000);
    break;
    }
  ...
  while(true){
    mpu.update();                                              
    if (mpu.getAngleX() > -5 && mpu.getAngleX() < 5){
      STOP();
      Serial.println("The car has stopped");
      delay(2000);
      break;
  ...
  double current_x_value2 = mpu.getAngleX();
  while(true){
  mpu.update();                                              
    if (mpu.getAngleX() > -5 && mpu.getAngleX() < 5 && mpu.getAngleX() < current_x_value2){
      STOP();
      Serial.println("The car has stopped");
      delay(2000);
      break;
  ...
  while (true){
    ...
    if (ultrasonic_distance_in_cm > 40){
        STOP();
        delay(2000);
        Serial.print("Car stopped"); 
      }
